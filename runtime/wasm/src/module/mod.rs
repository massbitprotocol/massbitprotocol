...
use massbit::runtime::{AscHeap, IndexForAscTypeId};
use massbit::{components::indexer::MappingError, runtime::AscPtr};
...
impl<C: Blockchain> WasmInstance<C> {
    ...
    pub fn from_valid_module_with_ctx(
        ...
        if let Some(timeout) = timeout {
            ...
            massbit::spawn_allow_panic(async move {
...
impl<C: Blockchain> WasmInstanceContext<C> {
    ...
    pub fn store_get(
        ...
        let ret = match entity_option {
            Some(entity) => asc_new(self, &entity.sorted())?,
            None => AscPtr::null(),
        };
    ...
    pub fn bytes_to_string(
        ...
        let string = host_exports::bytes_to_string(asc_get(self, bytes_ptr)?);
        asc_new(self, &string)
    }
