...
use massbit_data::prelude::{q, QueryExecutionError, StoreError};
use massbit_data::schema::FulltextAlgorithm;
use massbit_data::store::chain::BlockNumber;
use massbit_data::store::entity::{
    Attribute, AttributeNames, ChildMultiplicity, Entity, EntityCollection, EntityFilter,
    EntityKey, EntityLink, EntityOrder, EntityRange, EntityType, EntityWindow, ParentLink,
};
...
impl FromEntityData for Entity {
    type Value = massbit_data::store::Value;
...
impl<'a> QueryFragment<Pg> for InsertQuery<'a> {
    fn walk_ast(&self, mut out: AstPass<Pg>) -> QueryResult<()> {
        ...
        while let Some(entity) = iter.next() {
            ...
            out.push_bind_param::<Range<BigInt>, _>(&block_range)?;
            out.push_sql(")");
    ...
    S: AsRef<str> + diesel::serialize::ToSql<Text, Pg>,
{
    fn walk_ast(&self, mut out: AstPass<Pg>) -> QueryResult<()> {
        // update table
        //    set block_range = int8range(lower(block_range), $block)
...
impl<'a> QueryFragment<Pg> for RevertClampQuery<'a> {
    fn walk_ast(&self, mut out: AstPass<Pg>) -> QueryResult<()> {
        ...
        // Construct a query
        //   update table
        //     set block_range = int8range(lower(block_range), null)
        //   where block_range @> $block
        ...
        out.push_sql(" and coalesce(upper(");
        out.push_sql(BLOCK_RANGE_COLUMN);
        out.push_sql("), 9223372036854775807) < 9223372036854775807");
        out.push_sql("\nreturning ");
        out.push_sql(PRIMARY_KEY_COLUMN);
        out.push_sql("::text");
        Ok(())
