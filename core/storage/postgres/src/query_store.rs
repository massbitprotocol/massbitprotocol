...
use massbit_common::prelude::{anyhow::Error, async_trait::async_trait, tokio};
...
use massbit_data::indexer::DeploymentState;
use massbit_data::prelude::q;
use massbit_data::store::{PoolWaitStats, QueryStore as QueryStoreTrait, StoreError};
...
impl QueryStore {
    pub(crate) fn new(store: Arc<DeploymentStore>, site: Arc<Site>, replica_id: ReplicaId) -> Self {
        QueryStore {
            site,
            replica_id,
            store,
...
impl QueryStoreTrait for QueryStore {
    ...
    //                     "subgraph {} belongs to network {} but block {:?} belongs to network {}",
    ...
    async fn has_non_fatal_errors(&self, block: Option<BlockNumber>) -> Result<bool, StoreError> {
        let hash = self.site.deployment.clone();
        self.store
            .with_conn(move |conn, _| {
                crate::deployment::has_non_fatal_errors(conn, &hash, block).map_err(|e| e.into())
            })
            .await
    }
    ...
    fn api_schema(&self) -> Result<Arc<ApiSchema>, QueryExecutionError> {
        let info = self.store.indexer_info(&self.site)?;
        Ok(info.api)
    }
