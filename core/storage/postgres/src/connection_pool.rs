...
use massbit_common::{
    prelude::{
        ...
        lazy_static,
        slog::{crit, debug, error, info, o, Logger},
        tokio::{sync::Semaphore, time::Instant},
    },
    util::security::SafeDisplay,
};

use massbit_common::prelude::prometheus::{Counter, Gauge};
use massbit_common::util::{task_spawn, MovingStats};
use massbit_data::metrics::MetricsRegistry;
use massbit_data::prelude::StoreError;
use massbit_data::store::PoolWaitStats;
use std::fmt;
use std::str::FromStr;
use std::sync::Arc;
...
use massbit_data::utils::futures::{CancelGuard, CancelHandle, CancelToken, CancelableError};
...
impl ConnectionPool {
    ...
    pub(crate) async fn with_conn<T: Send + 'static>(
        ...
        let result = task_spawn::spawn_blocking_allow_panic(move || {
