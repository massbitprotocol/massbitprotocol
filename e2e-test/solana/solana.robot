...
###########################
Deploy substrate example test-solana-five-tables, then check if data exists in DB
    # Configuration
    ...
    Check If Exists In Database  SELECT * FROM transaction-account FETCH FIRST ROW ONLY
...
############################
Deploy substrate example test-solana-index-serum, then check if data exists in DB
    # Configuration
    Connect To Database  psycopg2  graph-node  graph-node  let-me-in  localhost  5432
