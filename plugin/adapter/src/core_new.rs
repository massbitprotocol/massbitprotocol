...
pub use crate::{
    handle_rust_mapping, handle_wasm_mapping, HandlerProxyType, PluginRegistrar,
    WasmHandlerProxyType,
};
        ...
        match data_sources.get(0) {
            Some(datasource) => {
                ...
                while let Some(data) = stream.message().await? {
                    ...
                    match ChainType::from_i32(data.chain_type).unwrap() {
                        ChainType::Solana => {
                ...
                match datasource.mapping.language.as_str() {
                    ...
                    //Default use rust//use massbit_runtime_wasm::manifest::{DataSource, DataSourceTemplate, Mapping, TemplateSource};
