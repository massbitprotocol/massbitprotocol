...
impl ForeignServer {
    ...
    /// accessible
    pub fn name(shard: &ShardName) -> String {
    ...
    pub fn metadata_schema(shard: &ShardName) -> String {
        format!("{}_subgraphs", Self::name(shard))
    }

    fn new(shard: ShardName, postgres_url: &str) -> Result<Self, anyhow::Error> {
    ...
    fn map_primary(conn: &PgConnection, shard: &ShardName) -> Result<(), StoreError> {
        let query = format!(
...
impl ConnectionPool {
    pub fn create(
        ...
        ConnectionPool {
            logger: logger_pool,
            shard: ShardName::new(shard_name.to_string())
    ...
    */
    ...
    fn configure_fdw(&self, servers: &Vec<ForeignServer>) -> Result<(), StoreError> {
        ...
        conn.transaction(|| {
            let current_servers: Vec<String> = super::catalog::current_servers(&conn)?;
