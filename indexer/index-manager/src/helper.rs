...
pub async fn loop_blocks(params: DeployParams) -> Result<(), Box<dyn Error>> {
    ...
    // create_new_indexer_detail_table(&connection, &index_config.query); # Old version of migration
    plugin_migration(&params.index_name, &index_config.schema, &index_config.config);



    // Track the newly created table with hasura
    // track_hasura_table(&params.table_name).await; # Old version of hasura API
    plugin_hasura(&params.index_name).await;



    //
    // Comment this until we have implemented the ddl gen migration and hasura API
    //
