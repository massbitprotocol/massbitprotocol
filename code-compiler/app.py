...
#   Constants  #
################
INDEXER_HOST = 'http://localhost:3030'
...
DEPLOY_BUILD_ERROR = 'build_error'
DEPLOY_BUILD_IN_PROGRESS = 'build_in_progress'
...
def compile_status(id):
    """
    compile_status check compilation status with compilation id

    :param id: (String) Compilation id

    :return: (String, String)
    """
    # Where we'll be looking for the compilation status
    generated_folder = "generated/" + id
    file = None
    status = None
    try:
...
def indexer(index_name, yaml_hash, lib_hash, model_hash):
    """
    indexer make api to indexer server

    :param index_name: (String) Index name for this index
    :param yaml_hash: (String) Hash of yaml file
    :param lib_hash: (String) Hash of dynamic library file
    :param model_hash: (String) Hash of model file
    ...
    res = requests.post(INDEXER_HOST, json=payload)
...
@app.route("/deploy/<compilation_id>", methods=['POST'])
@cross_origin()
def deploy_handler(compilation_id):
    # Check build with compilation_id, if `error` or `in_progress` return to user.
    status, build_log = compile_status(compilation_id)

    if status == BUILD_ERROR:
        return {
            "status": DEPLOY_BUILD_ERROR,
            "log": build_log,
        }, 200
    
    if status == BUILD_IN_PROGRESS:
        return {
            "status": DEPLOY_BUILD_IN_PROGRESS,
        }, 200

    # Build is success, grab them and upload to IPFS.
    model_file = f'generated/{compilation_id}/src/up.sql'
    yaml_file = f'generated/{compilation_id}/src/project.yaml'
    lib_file = f'generated/{compilation_id}/target/release/libblock.so'

    # On `darwin` it will build dynamic library to `.dylib` instead of `.so`
    if platform == 'darwin':
        lib_file = f'generated/{compilation_id}/target/release/libblock.dylib'

    with ipfshttpclient.connect() as client:
        model_hash = client.add(model_file).get('Hash', None)
        yaml_hash = client.add(yaml_file).get('Hash', None)
        lib_hash = client.add(lib_file).get('Hash', None)

    if (not model_hash) or (not yaml_hash) or (not lib_hash):
        return {
           "status": DEPLOY_ERROR,
           "payload": "fail to interact with IPFS service"
        }, 200

    status = indexer(
        index_name=compilation_id,
        yaml_hash=yaml_hash,
        lib_hash=lib_hash,
    ...
    if status != 200:
        return {
           "status": DEPLOY_ERROR,
           "payload": "fail to interact with indexer service"
        }, 200
...
def compile_status_handler(id):
    status, payload = compile_status(id)
    return {
        "status": status,
        "payload": payload
    }, 200
