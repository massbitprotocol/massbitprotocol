...
impl DataSource {
    ...
    fn handler_for_block(
        &self,
        trigger_type: &SolanaBlockTriggerType,
    ...
    fn matches_trigger_address(&self, trigger: &SolanaTriggerData) -> bool {
        true
    ...
    fn match_and_decode(
        &self,
        trigger: &SolanaTriggerData,
        block: Arc<EncodedConfirmedBlock>,
        logger: &Logger,
    ) -> Result<Option<SolanaMappingTrigger>, Error> {
        todo!()
...
impl TryFrom<DataSourceTemplateInfo<Chain>> for DataSource {
    ...
    fn try_from(info: DataSourceTemplateInfo<Chain>) -> Result<Self, anyhow::Error> {
        ...
        let address = String::from(string);
