...
async fn main() {
    ...
    let eth_networks = create_ethereum_networks(config)
        .await
    ...
    let launch_services = || async move {
        let (eth_networks, idents) = connect_networks(eth_networks).await;

        let store = store_builder.store();
        let mut blockchain_map = BlockchainMap::new();
        let ethereum_chains =
            networks_as_chains(&mut blockchain_map, node_id.clone(), &eth_networks);
        ...
        let indexer_provider =
            IndexerAssignmentProvider::new(link_resolver.cheap_clone(), indexer_instance_manager);
    ...
    std::thread::spawn(move || loop {
        ...
        if futures::executor::block_on(ping_send.clone().send(pong_send)).is_err() {
            debug!("Shutting down contention checker thread");
            break;
        }
...
async fn create_ethereum_networks(config: Config) -> Result<EthereumNetworks, anyhow::Error> {
    ...
    let statuses = join_all(
        eth_networks
            ...
            .map(|(network, eth_adapter)| async move {
                info!("Connecting to Ethereum to get network identifier");
                    ...
                    .map_err(Error::from)
                {
                    ...
                    // on the URL)
                    Ok(Err(e)) | Err(e) => {
                        error!("Connection to provider failed. Not using this provider");
                        Status::Broken {
                            network,
