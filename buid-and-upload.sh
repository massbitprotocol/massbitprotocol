...
docker build --tag sprise/dashboard:0.3 -f frontend/dashboard/Dockerfile -t dashboard . --no-cache; # We need no-cache because we're pulling from github master, or it won't update our new code
...
docker push sprise/dashboard:0.3;